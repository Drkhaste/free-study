...
const API = {
  async request(method, path, body = null) {
    const isWpApi = path.startsWith('/wp-json/');
    const opts = {
      method,
      headers: {},
    ...
    if (isWpApi && typeof wpApiSettings !== 'undefined') {
      opts.headers['X-WP-Nonce'] = wpApiSettings.nonce;
    }
  ...
  get(p) { return this.request('GET', p.startsWith('http') ? p : `${p}`); },
  post(p, b) { return this.request('POST', p.startsWith('http') ? p : `${p}`, b); },
  put(p, b) { return this.request('PUT', p.startsWith('http') ? p : `${p}`, b); },
  patch(p, b) { return this.request('PATCH', p.startsWith('http') ? p : `${p}`, b); },
  del(p) { return this.request('DELETE', p.startsWith('http') ? p : `${p}`); },
};
...
async function router() {
  // check auth first
  if (typeof medEduData === 'undefined' || !medEduData.user) {
    window.location.href = medEduData ? medEduData.loginUrl : '/wp-login.php';
    return;
  }

  let path = window.location.pathname;

  // Support for WordPress admin and frontend page via query param or URL
  const urlParams = new URLSearchParams(window.location.search);
  const medPath = urlParams.get('med_path');
  if (medPath) {
    path = medPath;
  } else if (window.location.pathname.includes('/medical-dashboard')) {
      const parts = window.location.pathname.split('/medical-dashboard');
      path = parts[1] || '/dashboard';
  }

  state.user = medEduData.user;
...
function navigate(path) {
  // If it's a blog path, redirect to WordPress blog
  if (path.startsWith('/blog')) {
    window.location.href = medEduData.homeUrl + path;
    return;
  }
  ...
  if (url.pathname.includes('wp-admin')) {
    ...
    // Frontend dashboard
    const newUrl = window.location.origin + '/medical-dashboard' + path;
...
async function logout() {
  ...
  window.location.href = medEduData.logoutUrl;
}
...
function sidebarLink(href, label, path) {
  const urlParams = new URLSearchParams(window.location.search);
  const currentPath = urlParams.get('med_path') || '/dashboard';
...
Pages.dashboard = async function() {
  ...
  try {
    const [projects, cards] = await Promise.all([
      API.get('/projects'),
      API.get('/flashcards/stats/overview'),
    ]);
...
Pages.projectView = async function(id) {
  ...
  try {
    const [proj, topics] = await Promise.all([
      API.get(`/projects/${id}`),
      API.get(`/topics?project_id=${id}&limit=50`),
    ]);
...
function showHighlightMenu(x, y, topicId) {
  ...
  // Add "Create Flashcard" button
  const fcBtn = document.createElement('button');
  fcBtn.className = `px-3 h-8 rounded-full bg-brand-600 text-white text-[10px] font-bold hover:bg-brand-700 flex items-center gap-1`;
  fcBtn.innerHTML = `<svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/></svg> فلش‌کارت`;
  fcBtn.onclick = () => openFlashcardFromSelection(topicId);
  menu.appendChild(fcBtn);

  document.body.appendChild(menu);
}

window.openFlashcardFromSelection = function(topicId) {
  const selection = window.getSelection();
  const text = selection.toString().trim();
  removeHighlightMenu();

  // Get project_id for this topic
  API.get(`/topics/${topicId}`).then(data => {
    addFlashcardModal(data.topic.project_id, topicId, text);
  });
};
...
async function applyHighlight(colorClass, topicId) {
  ...
  try {
    ...
    // Save the new HTML
    const contentArea = document.getElementById('content-to-highlight');
    await API.patch(`/topics/${topicId}/highlight`, {
...
window.loadTopicFlashcards = async function(topicId, filter = 'all') {
  try {
    const data = await API.get(`/flashcards?topic_id=${topicId}&limit=100`);
...
window.answerTopicCard = async function(cardId, level, event, topicId) {
  ...
  try {
    await API.post(`/review/quick-answer`, { card_id: cardId, level });
...
window.publishTopic = async function(id) {
  ...
  try {
    await API.post(`/topics/${id}/publish`, {});
...
window.quickCreatePost = async function() {
  ...
  try {
    ...
    if (!blogProj) {
      blogProj = (await API.post('/projects', { title: 'وبلاگ', description: 'پست‌های مستقیم وبلاگ', color: '#ec4899' })).project;
    }
    const title = prompt('عنوان پست جدید:');
    if (!title) return;

    const res = await API.post('/topics', { title, project_id: blogProj.id, status: 'draft' });
    navigate(`/topics/${res.topic.id}/edit`);
  } catch (err) { toast(err.message, 'error'); }
};
...
window.saveTopic = async function(id, projectId) {
  ...
  try {
    let res;
    if (id) res = await API.put(`/topics/${id}`, data);
    else res = await API.post('/topics', { ...data, project_id: finalProjectId });
...
window.generateWithAI = async function(topicId) {
  ...
  try {
    const res = await API.post('/ai/generate-topic', { title, topic_id: topicId || null });
...
window.addFlashcardModal = function(projectId = null, topicId = null, initialFront = '') {
  const modal = document.createElement('div');
  modal.className = 'fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4 scale-in';
  modal.innerHTML = `
    <div class="bg-white dark:bg-slate-800 rounded-2xl w-full max-w-md p-6 shadow-2xl modal-mobile-full">
      ...
      <form onsubmit="saveFlashcard(event)" class="space-y-4">
        ...
        <div>
          ...
          <textarea name="front" required rows="2" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-brand-500 outline-none" placeholder="مثلاً: تعریف نارسایی قلبی؟">${escapeHtml(initialFront)}</textarea>
        </div>
        <div>
  ...
  if (!projectId || !topicId) {
    API.get('/topics?limit=200').then(data => {
...
window.doImportCSV = async function(passedProjectId = null, passedTopicId = null) {
  ...
  try {
    const res = await API.post('/flashcards/import-csv', {
...
window.generateNewTopicAI = async function(e) {
  ...
  try {
    ...
    const resTopic = await API.post('/topics', { project_id: projectId, title, status: 'draft' });
    const topicId = resTopic.topic.id;

    // 2. Generate content with AI
    await API.post('/ai/generate-topic', { title, topic_id: topicId });
...
window.resetCard = async function(id) {
  ...
  await API.post(`/flashcards/${id}/reset`, {});
  toast('ریست شد', 'info');
  if (window._reloadFlashcards) window._reloadFlashcards();
};

window.exportFlashcards = function() {
  window.location.href = `${medEduData.root}medical-edu/v1/export/flashcards.csv?_wpnonce=${medEduData.nonce}`;
};
...
Pages.review = async function() {
  ...
  try {
    const data = await API.get('/review/queue?limit=50');
...
function renderReviewCard() {
  if (state.reviewIndex >= state.reviewQueue.length) {
    ...
    `;
    API.post(`/review/${state.reviewSessionId}/end`, {}).catch(() => {});
    return;
  }
...
window.answerCard = async function(button) {
  const card = state.reviewQueue[state.reviewIndex];
  try {
    const res = await API.post(`/review/${state.reviewSessionId}/answer`, { card_id: card.id, button });
...
Pages.ai = async function() {
  ...
  API.get('/ai/logs?limit=5').then(data => {
...
Pages.calendar = async function() {
  ...
  async function renderCalendar(monthStr) {
    ...
    const { tasks } = await API.get(`/tasks?month=${gregorianMonth}`);
    const taskMap = {};
    tasks.forEach(t => {
  ...
  window.saveTask = async (e) => {
    ...
    try {
      ...
      const { tasks } = await API.get(`/tasks?date=${data.task_date}`);
  ...
  window.deleteTask = async (id, jalaliDate) => {
    ...
    try {
      ...
      renderCalendar(currentMonth);
      const gregDate = moment(jalaliDate, 'jYYYY/jMM/jDD').format('YYYY-MM-DD');
      const { tasks } = await API.get(`/tasks?date=${gregDate}`);
    ...
    <div class="space-y-6 max-w-2xl">
      ...
      <div class="bg-white dark:bg-slate-800 rounded-2xl p-5 md:p-6 border border-slate-100 dark:border-slate-700">
        ...
        <div class="flex gap-3 flex-wrap">
          <button onclick="window.location.href=`${medEduData.root}medical-edu/v1/export/backup.json?_wpnonce=${medEduData.nonce}`" class="px-4 py-2 bg-slate-100 dark:bg-slate-700 rounded-lg text-sm hover:bg-slate-200">دانلود فایل پشتیبان (JSON)</button>
        </div>
      </div>
...
window.setupWebhook = async function() {
  try {
    const res = await API.post('/settings/telegram/setup-webhook', {});
    if (res.ok) {
...
window.deleteWebhook = async function() {
  ...
  try {
    const res = await API.post('/settings/telegram/delete-webhook', {});
    if (res.ok) {
...
window.checkWebhook = async function() {
  ...
  if (!statusEl) return;
  try {
    const res = await API.get('/settings/telegram/webhook-status');
